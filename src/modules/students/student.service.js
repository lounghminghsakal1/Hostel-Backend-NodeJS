...
const createStudent = async (studentRequestBody, hostelId) => {
  const {
    email,
    studentName,
    contactNumber,
  ...
  return await prisma.$transaction(async (tx) => {
    ...
    const department = await StudentRepository.findDepartmentById(tx, departmentId);
    ...
    const hostel = await StudentRepository.findHostelById(tx, hostelId);
    if (!hostel) throw createHttpError(404, "Hostel not found", { errors: "Invalid hostel id" });
    if (hostel.collegeId !== department.collegeId) throw createHttpError(409, "Invalid department or you cannot create student for this department", { errors: "Invalid request" });

    if (roomId) {
      //check room id is valid or not
      const room = await StudentRepository.findRoomById(tx, roomId);
      ...
      if (room.hostelId !== hostelId) throw createHttpError(409, "Room does not belongs to your hostel", { errors: "Room doesn't belongs to logged admin's hostel" });
    ...
    const studentRole = await StudentRepository.findRoleById(tx, "STUDENT");
    ...
    const createdUser = await StudentRepository.createUser(tx, email, tempPassword, studentRole.id);

    //user created now studentprofile has to be created
    const createdStudentProfile = await StudentRepository.createStudentProfile(tx, studentName, contactNumber, parentMobileNumber, createdUser.id, departmentId, roomId);
...
const getAllStudentProfiles = async () => {
  return await StudentRepository.getAllStudentProfiles();
};

const getOneStudentProfileById = async (id) => {
  const oneStudent = await StudentRepository.getOneStudentProfileById(id);
  if (oneStudent === null) throw createHttpError(404, `Student with id ${id} is not found`, { errors: "Student id not found in the database" });
  return oneStudent;
};

const updateStudentProfile = async (id, updateStudentProfileRequestBody, hostelId) => {
  const {
    email,
    studentName,
    contactNumber,
  ...
  //check studentprofile with that id is present in db or not , then only we can update it
  const studentProfile = await StudentRepository.findStudentProfileById(id);
  if (!studentProfile) throw createHttpError(404, `Student profile with id ${id} not found`, { errors: "Invalid id" });

  //check whether this student's status is active or not -> if active then only can update else not
  ...
  return await prisma.$transaction(async (tx) => {
    ...
    if(departmentId) {
      ...
      const hostelOfLoggedInHostelAdmin = await StudentRepository.findHostelById(tx, hostelId);
      if(!hostelOfLoggedInHostelAdmin) throw createHttpError(404, "Hostel id is not found", "Invalid hostel id of logged in admin");
      
      if(department.collegeId !== hostelOfLoggedInHostelAdmin.collegeId) throw createHttpError(409, "Invalid college id passed, you cannot change this student profile to this college id", {errors: "Hostel admin's hostel doesn't belongs to this college id"});
    }
    ...
    if(roomId) {
      ...
      if(room.hostelId !== hostelId) throw createHttpError(409, "You cannot assign room which is not belongs to your hostel", {errors: "Invalid room id"});
...
const updateStatusOfStudent = async (studentId, status) => {
  //check student with id is present or not
  const student = await StudentRepository.findStudentProfileById(studentId);
  ...
  const updatedUserProfileOfThatStudent = await StudentRepository.updateStatusOfStudent(student.user.id, status);
