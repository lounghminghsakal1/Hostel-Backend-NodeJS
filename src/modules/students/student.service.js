...
const createStudent = async (studentRequestBody, accessContext) => {
  ...
  return await prisma.$transaction(async (tx) => {
    ...
    const hostel = await StudentRepository.findHostelById(tx, accessContext.loggedAdminHostelId);
    if (!hostel) throw createHttpError(404, "Hostel not found", { errors: "Invalid hostel id" });
    if ((hostel.collegeId !== department.collegeId) || (department.collegeId !== accessContext.loggedInAdminCollegeId)) throw createHttpError(409, "Invalid department or you cannot create student for this department", { errors: "Invalid request" });

    if (roomId) {
      //check room id is valid or not - room id should be find within that hostel scope so 
      const room = await StudentRepository.findRoomById(tx, roomId, accessContext.loggedAdminHostelId);
      ...
      if (room.hostelId !== accessContext.loggedAdminHostelId) throw createHttpError(409, "Room does not belongs to your hostel", { errors: "Room doesn't belongs to logged admin's hostel" });
    ...
    const createdStudentProfile = await StudentRepository.createStudentProfile(tx, studentName, contactNumber, parentMobileNumber, createdUser.id, departmentId, roomId, accessContext.loggedInAdminCollegeId, accessContext.loggedAdminHostelId);
...
const getAllStudentProfiles = async (accessContext) => {
  return await StudentRepository.getAllStudentProfiles(accessContext.loggedAdminHostelId, accessContext.loggedInAdminCollegeId);
};

const getOneStudentProfileById = async (id, accessContext) => {
  const oneStudent = await StudentRepository.getOneStudentProfileById(id, accessContext.loggedAdminHostelId, accessContext.loggedInAdminCollegeId);
...
const updateStudentProfile = async (id, updateStudentProfileRequestBody, accessContext) => {
  ...
  const studentProfile = await StudentRepository.findStudentProfileById(id, accessContext.loggedAdminHostelId, accessContext.loggedInAdminCollegeId);
  ...
  return await prisma.$transaction(async (tx) => {
    ...
    if(departmentId) {
      ...
      // const hostelOfLoggedInHostelAdmin = await StudentRepository.findHostelById(tx, accessContext.loggedAdminHostelId);
    ...
    if(roomId) {
      ...
      if(room.hostelId !== accessContext.loggedAdminHostelId) throw createHttpError(409, "You cannot assign room which is not belongs to your hostel", {errors: "Invalid room id"});
...
const updateStatusOfStudent = async (studentId, status, accessContext) => {
  ...
  const student = await StudentRepository.findStudentProfileById(studentId, accessContext.loggedAdminHostelId, accessContext.loggedInAdminCollegeId);
  ...
  return StudentRepository.findStudentProfileById(studentId,accessContext.loggedAdminHostelId, accessContext.loggedInAdminCollegeId);
};

const changeOrAssignStudentRoom = async (studentId, roomId, accessContext) => {
  //check student with id present or not
  const studentProfile = await StudentRepository.findStudentProfileById(studentId, accessContext.loggedAdminHostelId, accessContext.loggedInAdminCollegeId);
  ...
  const room = await StudentRepository.findRoomById(prisma, roomId, accessContext.loggedAdminHostelId);
  ...
  if(studentProfile.room && studentProfile.room.hostelId !== accessContext.loggedAdminHostelId) throw createHttpError(409, "The student doen't belongs to your hostel", {errors: "Invalid student id"});

  //room.hostel id must match with logged admin hostel id
  if(room.hostelId !== accessContext.loggedAdminHostelId) throw createHttpError(409, `This room doesn't belongs to your hostel`, {errors: "Invalid room id"});
